...
from finance.tasks.email_tasks import send_yearly_summaries
from finance.enums import TransactionType, EmailType


class YearlySummaryTaskTests(TestCase):
    def setUp(self):
    ...
    def test_send_yearly_summaries_sends_to_eligible_users(self):
        result = send_yearly_summaries()
    ...
    def test_email_sent_has_correct_subject(self):
        ...
        self.assertEqual(mail.outbox[0].subject, f"Your {year} Year in Review")
    ...
    def test_email_includes_yearly_overview(self):
        send_yearly_summaries()

        self.assertIn("Total Income:", mail.outbox[0].body)
        self.assertIn("Total Expenses:", mail.outbox[0].body)
        self.assertIn("Net Income:", mail.outbox[0].body)

    def test_creates_email_log_for_successful_send(self):
        send_yearly_summaries()

        log = EmailLog.objects.get(user=self.user_with_transactions)
        self.assertEqual(log.email_type, EmailType.YEARLY_SUMMARY.name)
    ...
    def test_handles_multiple_eligible_users(self):
        ...
        UserSettings.objects.create(user=user4, yearly_summary_enabled=True)
        Transaction.objects.create(
            user=user4,
    ...
    def test_excludes_users_with_only_previous_year_transactions(self):
        ...
        UserSettings.objects.create(user=user5, yearly_summary_enabled=True)
        Transaction.objects.create(
            user=user5,
            type=TransactionType.NEED.name,
