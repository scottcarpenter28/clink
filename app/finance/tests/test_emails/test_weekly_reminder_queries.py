from django.test import TestCase
from django.contrib.auth.models import User
from django.utils import timezone
from datetime import timedelta

from finance.models import UserSettings, Transaction
from finance.emails.weekly_reminder.queries import get_users_needing_reminders
from finance.enums import TransactionType


class WeeklyReminderQueriesTests(TestCase):
    def setUp(self):
        self.user_with_settings = User.objects.create_user(
            username="user_with_settings",
            email="user1@example.com",
            password="testpass123",
        )
        UserSettings.objects.create(
            user=self.user_with_settings, weekly_reminder_enabled=True
        )

        self.user_disabled = User.objects.create_user(
            username="user_disabled",
            email="user2@example.com",
            password="testpass123",
        )
        UserSettings.objects.create(
            user=self.user_disabled, weekly_reminder_enabled=False
        )

        self.user_no_email = User.objects.create_user(
            username="user_no_email", email="", password="testpass123"
        )
        UserSettings.objects.create(
            user=self.user_no_email, weekly_reminder_enabled=True
        )

        self.user_with_recent_transaction = User.objects.create_user(
            username="user_recent",
            email="user3@example.com",
            password="testpass123",
        )
        UserSettings.objects.create(
            user=self.user_with_recent_transaction, weekly_reminder_enabled=True
        )

    def test_get_users_needing_reminders_returns_users_without_recent_transactions(
        self,
    ):
        users = get_users_needing_reminders()

        self.assertIn(self.user_with_settings, users)
        self.assertIn(self.user_with_recent_transaction, users)
        self.assertEqual(users.count(), 2)

    def test_excludes_users_with_disabled_reminders(self):
        users = get_users_needing_reminders()

        self.assertNotIn(self.user_disabled, users)

    def test_excludes_users_without_email(self):
        users = get_users_needing_reminders()

        self.assertNotIn(self.user_no_email, users)

    def test_excludes_users_with_recent_transactions(self):
        Transaction.objects.create(
            user=self.user_with_recent_transaction,
            type=TransactionType.NEED.name,
            category="Groceries",
            amount_in_cents=5000,
            date_of_expense=timezone.now().date(),
        )

        users = get_users_needing_reminders()

        self.assertNotIn(self.user_with_recent_transaction, users)

    def test_includes_users_with_old_transactions(self):
        Transaction.objects.create(
            user=self.user_with_settings,
            type=TransactionType.NEED.name,
            category="Groceries",
            amount_in_cents=5000,
            date_of_expense=timezone.now().date() - timedelta(days=8),
        )

        users = get_users_needing_reminders()

        self.assertIn(self.user_with_settings, users)

    def test_excludes_users_with_transactions_exactly_7_days_ago(self):
        Transaction.objects.create(
            user=self.user_with_settings,
            type=TransactionType.NEED.name,
            category="Groceries",
            amount_in_cents=5000,
            date_of_expense=timezone.now().date() - timedelta(days=6),
        )

        users = get_users_needing_reminders()

        self.assertNotIn(self.user_with_settings, users)
